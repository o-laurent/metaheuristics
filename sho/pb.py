...
def old_coverage(domain, sensors, sensor_range):
...
def coverage(domain, sensors, sensor_range):
    """Homemade faster coverage function by Arthur Liu. """
    n = domain.shape[0]
    m = domain.shape[1]

    Y = np.tile(np.arange(m), (n, 1))
    X = np.tile(np.arange(n), (m, 1)).transpose()

    for s in sensors:
        domain += (np.sqrt(np.square(s[0] - Y) +
                   np.square(s[1] - X)) < sensor_range)
