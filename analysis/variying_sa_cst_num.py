...
eval_function = make.func(num.cover_sum,
                          domain_width=domain_width,
                          sensor_range=sensor_range,
                          dim=d * nb_sensors
                          )
init_function = make.init(num.rand_init_halt,
                          scale=domain_width,
                          dim=d*nb_sensors)
neigh_function = make.neig(num.neighb_square,
...
dataframes = []
for max_iter in range(100, 1601, 100):
    whole_history = []
    for _ in range(mean_nb):
        val, sol = algo.simulated_annealing(
            ...
            make.temp(temperature_func, max_iter=max_iter),
        )
...
with open('dataframes_init_high_num_stuck_not_capped.pickle', 'wb') as handle:
