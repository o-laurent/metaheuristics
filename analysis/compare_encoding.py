...
for df in dataframes:
    ...
    print(df)
    if not done:
        sns.lineplot(data=df, x="level_0", y=0, color="b", label="Bit Encoding")
    else:
...
with open('dataframes_init_high_num_stuck.pickle', 'rb') as handle:
    dataframes = pickle.load(handle)
...
for df in dataframes:
    ...
    print(df)
    if not done:
        sns.lineplot(data=df, x="level_0", y=0, color="r", ax=ax, label="Num Encoding")
    else:
...
fig.savefig("analysis/encoding_notcapped.png")
plt.show()